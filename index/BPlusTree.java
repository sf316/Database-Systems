...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanAll() {
        ...
        BPlusTreeIterator iter = new BPlusTreeIterator(root.getLeftmostLeaf());
        return iter;
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        BPlusTreeIterator iter = new BPlusTreeIterator(root.get(key), key);
        return iter;
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> p = root.put(key, rid);
        if (p.isPresent()){
            DataBox k = p.get().getFirst();
            long s = p.get().getSecond();
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        if (!(root instanceof LeafNode && !(node.scanAll().hasNext())) ) {
        ...
        while (data.hasNext()){
            Optional<Pair<DataBox, Long>> p = root.bulkLoad(data, fillFactor);
            if (p.isPresent()){
                DataBox k = p.get().getFirst();
                long s = p.get().getSecond();
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator (LeafNode start){
               this.iter = start.scanAll();
               this.start = start;
        }

        public BPlusTreeIterator (LeafNode start, DataBox key){
               this.iter = start.scanGreaterEqual(key);
               this.start = start;
        }
        @Override
        public boolean hasNext() {
            ...
            if (iter != null){              
                ...
                Optional<LeafNode> rightSibling = start.getRightSibling();

                while(rightSibling.isPresent()){
                    start = rightSibling.get();
                    iter = start.scanAll();
                    ...
                    rightSibling = start.getRightSibling();
                }            
            }
