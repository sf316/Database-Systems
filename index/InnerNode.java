...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        int n = this.numLessThanEqual(key, keys);
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        int j = this.numLessThanEqual(key, keys);
        BPlusNode node = getChild(j);
        Optional<Pair<DataBox, Long>> p = node.put(key, rid);

        if (p.isPresent()){
            DataBox splitedKey = p.get().getFirst();
            Long n = p.get().getSecond();

            if (keys.contains(splitedKey)){
                return Optional.empty();
            }
            
            int index = this.numLessThan(splitedKey, keys);
            keys.add(index, splitedKey);
            children.add(index + 1, n);
            sync();
            
            if (children.size() > (int)2*d + 1){
                List<DataBox> leftKeys = new ArrayList<>();
                List<Long> leftChildren = new ArrayList<>();
                List<DataBox> rightKeys = new ArrayList<>();
                List<Long> rightChildren = new ArrayList<>();

                DataBox middleKey = keys.remove(d);

                for (int i = 0; i < d; i ++){
                    leftKeys.add(keys.get(i));
                    leftChildren.add(children.get(i));
                } 
                leftChildren.add(children.get(d));

                for (int i = d; i < 2*d; i ++){
                    rightKeys.add(keys.get(i));
                    rightChildren.add(children.get(i+1));
                }
                rightChildren.add(children.get(2*d + 1));
                
                InnerNode rightNode = new InnerNode(metadata, bufferManager, rightKeys, rightChildren, treeContext);
                keys = leftKeys;
                children = leftChildren;

                long num = rightNode.getPage().getPageNum();
                Pair<DataBox, Long> newSplitedKey = new Pair<>(middleKey, num);
                
                sync();
                return Optional.of(newSplitedKey);                
            }
        }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (data.hasNext()){
            //LeafNode rightMostNode = getLeftmostLeaf();
            //Optional<LeafNode> rightSibling = rightMostNode.getRightSibling();

            //while (rightSibling.isPresent()){
            //    rightMostNode = rightSibling.get();
            //    rightSibling = rightMostNode.getRightSibling();
            //}
            BPlusNode rightMostNode = getChild(children.size()-1);
            Optional<Pair<DataBox, Long>> p = rightMostNode.bulkLoad(data, fillFactor);
