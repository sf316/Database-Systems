...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (keys.size() > 2*d){
           ...
           long n = rightNode.getPage().getPageNum();
           keys = leftKeys;
           rids= leftRids;
           rightSibling = Optional.of(n);

           Pair<DataBox, Long> pair = new Pair<>(rightKeys.get(0), n);
           sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (data.hasNext()){
            ...
            if(keys.size() >= (int) Math.ceil(fillFactor*2*d)){
                 List<DataBox> rightKeys = new ArrayList<>();
                 List<RecordId> rightRids = new ArrayList<>();
                 ...
                 long n = rightNode.getPage().getPageNum();
                 this.rightSibling = Optional.of(n);
                 
                 Pair<DataBox, Long> pair2 = new Pair<>(rightKeys.get(0), n);
                 sync();
                 return Optional.of(pair2);            
            }else{
                 keys.add(pair.getFirst());
                 Collections.sort(keys);
                 int index = keys.indexOf(pair.getFirst());
