...
public class QueryPlan {
    ...
    public Iterator<Record> execute() {
        ...
        tables.add(this.startTableName);

        Map<Set, QueryOperator> map = new HashMap<>();
        QueryOperator lowestCost = this.minCostSingleAccess(this.startTableName);
        map.put(tables, lowestCost);

        for (int i = 0; i < this.joinTableNames.size(); i++){
            Set<String> set = new HashSet<>();
            set.add(this.joinTableNames.get(i));

            lowestCost = this.minCostSingleAccess(this.joinTableNames.get(i));
            map.put(set, lowestCost);
        }

        Map<Set, QueryOperator> previousMap = new HashMap<>(map);
        QueryOperator source = this.minCostOperator(previousMap);
        while (previousMap.size() > 1){
            previousMap = this.minCostJoins(previousMap, map);
            source = this.minCostOperator(previousMap);
        }
    ...
    QueryOperator minCostSingleAccess(String table) {
        ...
        minOp = new SequentialScanOperator(this.transaction, table);
        int seqCost = minOp.getIOCost();

        List<Integer> indexColumns = this.getEligibleIndexColumns(table);
        int except = -1;

        for (int indexColumn = 0; indexColumn < indexColumns.size(); indexColumn++){
            int selectIndex = indexColumns.get(indexColumn);
            PredicateOperator operator = this.selectOperators.get(selectIndex);
            DataBox value = this.selectDataBoxes.get(selectIndex);

            QueryOperator newOp = new IndexScanOperator(this.transaction, this.startTableName, this.selectColumnNames.get(selectIndex), operator, value);
    ...
    Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
        ...
        for (Map.Entry<Set, QueryOperator> tables : prevMap.entrySet()){
            for (String joinCondition : this.joinTableNames){
                String [] leftSide = this.getJoinLeftColumnNameByIndex(this.joinTableNames.indexOf(joinCondition));
                String [] rightSide = this.getJoinRightColumnNameByIndex(this.joinTableNames.indexOf(joinCondition));


                Set<String> set = tables.getKey();

                if(set.contains(leftSide[0]) && !set.contains(rightSide[0])) {
                    Set<String> r = new HashSet<>();
                    r.add(rightSide[0]);
                    newSet = new HashSet<>(r);
                    leftOperator = tables.getValue();
                    rightOperator = pass1Map.get(newSet);
                }else if(!set.contains(leftSide[0]) && set.contains(rightSide[0])){
                    Set<String> l = new HashSet<>();
                    l.add(leftSide[0]);
                    newSet = new HashSet<>(l);
                    leftOperator = pass1Map.get(newSet);
                    rightOperator = tables.getValue();
                }else{
                    continue;
                }

                newSet.addAll(set);
                cheapestJoin = minCostJoinType(leftOperator, rightOperator, leftSide[1], rightSide[1]);

                if (!map.containsKey(newSet) || cheapestJoin.getIOCost() < map.get(newSet).getIOCost()){
