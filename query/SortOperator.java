...
public class SortOperator {
    ...
    public Run sortRun(Run run) {
        ...
        Iterator<Record> input = run.iterator();
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        for (int i = 0; i < runs.size(); i++){
            Iterator<Record> rid = runs.get(i).iterator();

            if (rid.hasNext()){
                iters.add(rid);
                pq.add(new Pair<>(rid.next(), i));
            }
        }
        ...
        while (!pq.isEmpty()) {
            Pair<Record, Integer> removed = pq.remove();
            lst.add(removed.getFirst());

            Iterator<Record> iter = iters.get(removed.getSecond());

           if (iter.hasNext()){
                pq.add(new Pair<>(iter.next(), removed.getSecond()));
           }
        }
    ...
    public List<Run> mergePass(List<Run> runs) {
        // TODO(proj3_part1): implement
        List<Run> lst = new ArrayList<>();
        int num = this.numBuffers - 1;

        for (int i = 0; i < runs.size(); i+= num){
            List<Run> temp = new ArrayList<>();

            if(i + num > runs.size()){
                for(int j = i; j < runs.size()- ((int) (runs.size()/num))*num; j++){
                    temp.add(runs.get(i));
                }
            }else{
            ...
            lst.add(mergeSortedRuns(temp));
        }

        /*List<Run> temp = new ArrayList<>();
        for (int i = ((int) (runs.size()/num))*num; i < runs.size(); i++){
            temp.add(runs.get(i));
        }
        lst.add(mergeSortedRuns(temp));*/
        return lst;
    ...
    public String sort() {
        ...
        List<Run> lst = new ArrayList<>();
        //Iterator<Page> page = this.transaction.getPageIterator(tableName);
        BacktrackingIterator<Page> page = this.transaction.getPageIterator(this.tableName);

        while (page.hasNext()){
            Run newRun = createRunFromIterator(this.transaction.getBlockIterator(tableName, page, numBuffers));
            lst.add(sortRun(newRun));
        }

        if(lst.size() > 1){
            return mergePass(lst).get(0).tableName();
        }else{
            return lst.get(0).tableName();
        }
    }
